...
describe('Authorizer test suites', () =>{
    ...
    describe('login user', () => {
        ...
        test('should return null if invalid credential', async() =>{
            ...
            expect(userCredentialsDBAccessMock.getUserCredential).toBeCalledWith(someAccount.username, someAccount.password)
        });
    ...
    describe('validateToken test', () =>{
       ...
       test('validateToken returns valid', async () =>{

           sessionTokenDBAccessMock.getToken.mockReturnValue({valid: true, accessRights: ['']})
           const sessionToken = await authorizer.validateToken('123');
           expect(sessionToken).toStrictEqual({
            accessRights: [''],
